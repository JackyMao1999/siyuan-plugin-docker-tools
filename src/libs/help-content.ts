...
<tr><td>全部可访问内容</td><td>✅ 关键词留空</td><td>来源选「搜索有权限的文档」→ <b>关键词留空</b>即列出全部知识空间（不依赖搜索接口）</td></tr>
...
② 来源选「搜索有权限的文档」→ <b>关键词留空</b>可列出全部知识空间；输入关键词才走搜索接口，
而搜索接口按「<b>用户可见</b>」设计，<b>建议配合「用户身份」使用</b>（需 <code>search:docs:read</code>）。<br>
...
<dd>按顺序排查：
<ol>
<li><b>是不是没填关键词</b>：飞书搜索接口是按关键词检索的，<b>空关键词不作为「查全部」</b>。现在插件在关键词留空时会改为「列出全部知识空间」，不再走搜索接口。</li>
